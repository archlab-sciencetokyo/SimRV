...
public:
    VirtualTerminal(int w = 80, int h = 24) {
        resize(w, h);
    }
    ...
    void resize(int new_width, int new_height) {
        ...
        if (new_height < height_) {
            ...
            for (int i = 0; i < diff; ++i) {
                cells_.push_back(std::vector<Cell>(new_width, Cell{ .ch = " ", .fg = 7, .bg = current_attr_.bg }));
            }
        }
    ...
    [[nodiscard]] auto get_line_as_string(int idx, int width_limit, bool draw_cursor_at_x = false) const -> std::string {
    ...
    void set_scroll_offset_callback(std::function<void(int)> cb) {
        scroll_offset_cb_ = cb;
    }
...
private:
    ...
    void write_cell_string(const std::string& s) {
        if (cursor_y_ >= 0 && cursor_y_ < height_ && cursor_x_ >= 0 && cursor_x_ < width_) {
            cells_[cursor_y_][cursor_x_] = Cell{
                .ch = s,
                .fg = current_attr_.fg,
                .bg = current_attr_.bg,
    ...
    void execute_csi_command(char cmd) {
        ...
        if (cmd == 'm') { // SGR
            ...
            if (mode == 0) {
                for (int x = cursor_x_; x < width_; ++x) cells_[cursor_y_][x] = Cell{ .ch = " ", .fg = 7, .bg = current_attr_.bg };
                for (int y = cursor_y_ + 1; y < height_; ++y) {
                    std::fill(cells_[y].begin(), cells_[y].end(), Cell{ .ch = " ", .fg = 7, .bg = current_attr_.bg });
                }
            } else if (mode == 1) {
                for (int y = 0; y < cursor_y_; ++y) {
                    std::fill(cells_[y].begin(), cells_[y].end(), Cell{ .ch = " ", .fg = 7, .bg = current_attr_.bg });
                }
                for (int x = 0; x <= cursor_x_; ++x) cells_[cursor_y_][x] = Cell{ .ch = " ", .fg = 7, .bg = current_attr_.bg };
            } else if (mode == 2 || mode == 3) {
                for (auto& row : cells_) {
                    std::fill(row.begin(), row.end(), Cell{ .ch = " ", .fg = 7, .bg = current_attr_.bg });
                }
            }
            ...
            if (mode == 0) {
                ...
                std::fill(cells_[cursor_y_].begin(), cells_[cursor_y_].end(), Cell{ .ch = " ", .fg = 7, .bg = current_attr_.bg });
            }
