...
uint32_t CB_imm_gen (uint32_t ir){
    ...
    uint32_t uimm_B = ((ir >> 31) << 12) | (((ir >> 7) & 1) << 11) | 
    ...
    switch (ir & 0x7F) {
    case OPCODE_OP_IMM    : { ret = imm_I; break; }
    case OPCODE_LOAD      : { ret = imm_I; break; }
    case OPCODE_JALR      : { ret = imm_I; break; }
    case OPCODE_STORE     : { ret = imm_S; break; }
    case OPCODE_BRANCH    : { ret = imm_B; break; }
    case OPCODE_LUI       : { ret = imm_U; break; }
    case OPCODE_AUIPC     : { ret = imm_U; break; }
    case OPCODE_JAL       : { ret = imm_J; break; }
    case OPCODE_SYSTEM    : { ret = zimm;  break; }
    default               : { ret = 0;     break; }
    }
...
uint32_t decomp_c0 (uint32_t ir){
    ...
    uint32_t uimm1  = (((ir >>  5) & 0x1) << 6) |
                      (((ir >> 10) & 0x7) << 3) |
                      (((ir >>  6) & 0x1) << 2);
    uint32_t uimm2  = (((ir >>  5) & 0x3) << 6) |
                      (((ir >> 10) & 0x7) << 3);
    uint32_t nzuimm = (((ir >>  7) & 0xF) << 6) |
                      (((ir >> 11) & 0x3) << 4) |
                      (((ir >>  5) & 0x1) << 3) |
                      (((ir >>  6) & 0x1) << 2);
...
uint32_t decomp_c1 (uint32_t ir){
    ...
    uint32_t nzimm = (((ir >> 12) & 1) ? 0xFFFFFFE0 : 0x0) | (((ir >> 12) & 1) << 5) | ((ir >> 2) & 0x1F);
    uint32_t shamt = nzimm & 0x1F;
    uint32_t uimm1 = (((ir >> 12) & 0x1) << 11) |
                     (((ir >>  8) & 0x1) << 10) |
                     (((ir >>  9) & 0x3) <<  8) |
                     (((ir >>  6) & 0x1) <<  7) |
                     (((ir >>  7) & 0x1) <<  6) |
                     ...
                     (((ir >> 11) & 0x1) <<  4) |
                     (((ir >>  3) & 0x7) <<  1);
    uint32_t uimm2 = (((ir >> 12) & 0x1) <<  8) |
                     (((ir >>  5) & 0x3) <<  6) |
                     (((ir >>  2) & 0x1) <<  5) |
                     (((ir >> 10) & 0x3) <<  3) |
                     (((ir >>  3) & 0x3) <<  1);
    uint32_t uimm3 = (((ir >> 12) & 1) << 5)    |
                     ((ir >> 2) & 0x1F);
    uint32_t uimm4 = (((ir >> 12) & 0x1) << 9) |
                     (((ir >>  3) & 0x3) << 7) |
                     (((ir >>  5) & 0x1) << 6) |
                     (((ir >>  2) & 0x1) << 5) |
                     (((ir >>  6) & 0x1) << 4);
...
uint32_t decomp_c2 (uint32_t ir)
{
    ...
    uint32_t uimm1 = (((ir >> 2) & 0x7) << 6) | (((ir >> 12) & 0x1) << 5) | (((ir >> 5) & 0x3) << 3);
    uint32_t uimm2 = (((ir >> 2) & 0x3) << 6) | (((ir >> 12) & 0x1) << 5) | (((ir >> 4) & 0x7) << 2);
...
uint32_t CB_inst_decomp(uint32_t ir){
    switch (ir & 0x3) {
        case OPCODE_C0: return decomp_c0(ir & 0xFFFF);
        case OPCODE_C1: return decomp_c1(ir & 0xFFFF);
        case OPCODE_C2: return decomp_c2(ir & 0xFFFF);
    }
    return ir;
}
...
uint32_t ALU_IM(uint32_t in1, uint32_t in2, uint32_t funct3, uint32_t funct7){
    ...
    if ((funct7 & 0x1) == 0) {
        switch (funct3) {
            ...
            case FUNCT3_SRL : {
                if (funct7) ret = (int32_t)in1 >> shamt;
                else        ret = in1 >> shamt;
                break;
            }
            case FUNCT3_OR : {
        ...
        switch (funct3) {
            ...
            case FUNCT3_DIV : {
                if (in2 == 0xFFFFFFFF) ret = in1;
                else if (in2 == 0)     ret = 0xFFFFFFFF;
                else                   ret = (int32_t)in1 / (int32_t)in2;
                break;
            }
            case FUNCT3_DIVU : {
                if (in2 == 0) ret = 0xFFFFFFFF;
                else          ret = in1 / in2;
                break;
            }
            case FUNCT3_REM : {
                if (in2 == 0xFFFFFFFF) ret = 0;
                else if (in2 == 0)     ret = in1;
                else                   ret = (int32_t)in1 % (int32_t)in2;
                break;
            }
            case FUNCT3_REMU : {
...
uint32_t ALU_B(uint32_t in1, uint32_t in2, uint32_t funct3){
    uint32_t ret = 0;
    switch (funct3) {
    case FUNCT3_BEQ  : { ret = in1 == in2;                   break; }
    case FUNCT3_BNE  : { ret = in1 != in2;                   break; }
    case FUNCT3_BLT  : { ret = (int32_t)in1 <  (int32_t)in2; break; }
    case FUNCT3_BGE  : { ret = (int32_t)in1 >= (int32_t)in2; break; }
    case FUNCT3_BLTU : { ret = in1  < in2;                   break; }
    case FUNCT3_BGEU : { ret = in1 >= in2;                   break; }
    }
    return ret;
}
...
uint32_t ALU_A (uint32_t in1, uint32_t in2, uint32_t funct5){
    uint32_t ret = 0;
    switch (funct5) {
    case FUNCT5_AMO_LR   : { ret = 0;                                       break; }
    case FUNCT5_AMO_SC   : { ret = in1;                                     break; }
    case FUNCT5_AMO_SWAP : { ret = in1;                                     break; }
    case FUNCT5_AMO_ADD  : { ret = in1 + in2;                               break; }
    case FUNCT5_AMO_AND  : { ret = in1 & in2;                               break; }
    case FUNCT5_AMO_OR   : { ret = in1 | in2;                               break; }
    case FUNCT5_AMO_XOR  : { ret = in1 ^ in2;                               break; }
    case FUNCT5_AMO_MIN  : { ret = (int32_t)in1 < (int32_t)in2 ? in1 : in2; break; }
    case FUNCT5_AMO_MINU : { ret = in1 < in2 ? in1 : in2;                   break; }
    case FUNCT5_AMO_MAX  : { ret = (int32_t)in1 > (int32_t)in2 ? in1 : in2; break; }
    case FUNCT5_AMO_MAXU : { ret = in1 > in2 ? in1 : in2;                   break; }
    }
    return ret;
}
...
uint32_t ALU_C (uint32_t rcsr, uint32_t rrs1, uint32_t imm, uint32_t funct3){
    uint32_t ret = 0;
    switch (funct3) {
    case FUNCT3_CSRRW  : { ret = rrs1;           break; }
    case FUNCT3_CSRRS  : { ret = rcsr | rrs1;    break; }
    case FUNCT3_CSRRC  : { ret = rcsr & (~rrs1); break; }
    case FUNCT3_CSRRWI : { ret = imm;            break; }
    case FUNCT3_CSRRSI : { ret = rcsr | imm;     break; }
    case FUNCT3_CSRRCI : { ret = rcsr & (~imm);  break; }
    }
    return ret;
}
...
OPERATION_ID decoder (uint32_t ir){
    ...
    switch (opcode) {
        case OPCODE_LUI    : ret = LUI_______; break;
        case OPCODE_AUIPC  : ret = AUIPC_____; break;
        case OPCODE_JAL    : ret = JAL_______; break;
        case OPCODE_JALR   : ret = JALR______; break;
        case OPCODE_BRANCH : {
            switch (funct3) {
                case FUNCT3_BEQ  : ret = BEQ_______; break;
                case FUNCT3_BNE  : ret = BNE_______; break;
                case FUNCT3_BLT  : ret = BLT_______; break;
                case FUNCT3_BGE  : ret = BGE_______; break;
                case FUNCT3_BLTU : ret = BLTU______; break;
                case FUNCT3_BGEU : ret = BGEU______; break;
            }
            break;
        }
        case OPCODE_LOAD : {
            switch (funct3) {
                case FUNCT3_LB  : ret = LB________; break;
                case FUNCT3_LH  : ret = LH________; break;
                case FUNCT3_LW  : ret = LW________; break;
                case FUNCT3_LBU : ret = LBU_______; break;
                case FUNCT3_LHU : ret = LHU_______; break;
            }
            break;
        }
        case OPCODE_STORE : {
            switch (funct3) {
                case FUNCT3_SB  : ret = SB________; break;
                case FUNCT3_SH  : ret = SH________; break;
                case FUNCT3_SW  : ret = SW________; break;
            }
            break;
        }
        case OPCODE_OP_IMM : {
            switch (funct3) {
                case FUNCT3_ADD  : ret = ADDI______; break;
                case FUNCT3_SLT  : ret = SLTI______; break;
                case FUNCT3_SLTU : ret = SLTIU_____; break;
                case FUNCT3_XOR  : ret = XORI______; break;
                case FUNCT3_OR   : ret = ORI_______; break;
                case FUNCT3_AND  : ret = ANDI______; break;
                case FUNCT3_SLL  : ret = SLLI______; break;
                case FUNCT3_SRL  : {
                    if (funct7) {
                        ret = SRAI______; break;
                    } else {
                        ret = SRLI______; break;
                    }
                    break;
                }
            }
        ...
        case OPCODE_OP : {
            if (funct7 & 0x1) {
                switch (funct3) {
                    case FUNCT3_MUL    : ret = MUL_______; break;
                    case FUNCT3_MULH   : ret = MULH______; break;
                    case FUNCT3_MULHSU : ret = MULHSU____; break;
                    case FUNCT3_MULHU  : ret = MULHU_____; break;
                    case FUNCT3_DIV    : ret = DIV_______; break;
                    case FUNCT3_DIVU   : ret = DIVU______; break;
                    case FUNCT3_REM    : ret = REM_______; break;
                    case FUNCT3_REMU   : ret = REMU______; break;
                }
            } else {
                switch (funct3) {
                    case FUNCT3_ADD  : {
                        if (funct7) {
                            ret = SUB_______; break;
                        } else {
                            ret = ADD_______; break;
                        }
                        break;
                    }
                    case FUNCT3_SLT  : ret = SLT_______; break;
                    case FUNCT3_SLTU : ret = SLTU______; break;
                    case FUNCT3_XOR  : ret = XOR_______; break;
                    case FUNCT3_OR   : ret = OR________; break;
                    case FUNCT3_AND  : ret = AND_______; break;
                    case FUNCT3_SLL  : ret = SLL_______; break;
                    case FUNCT3_SRL  : {
                        if (funct7) {
                            ret = SRA_______; break;
                        } else {
                            ret = SRL_______; break;
                        }
                        break;
                    }
                }
        ...
        case OPCODE_MISC_M : {
            switch (funct3) {
                case FUNCT3_FENCE   : ret = FENCE_____; break;
                case FUNCT3_FENCE_I : ret = FENCE_I___; break;
            }
            break;
        }
        case OPCODE_AMO : {
            switch (funct5) {
                case FUNCT5_AMO_LR   : ret = LR_W______; break;
                case FUNCT5_AMO_SC   : ret = SC_W______; break;
                case FUNCT5_AMO_SWAP : ret = AMOSWAP_W_; break;
                case FUNCT5_AMO_ADD  : ret = AMOADD_W__; break;
                case FUNCT5_AMO_XOR  : ret = AMOXOR_W__; break;
                case FUNCT5_AMO_AND  : ret = AMOAND_W__; break;
                case FUNCT5_AMO_OR   : ret = AMOOR_W___; break;
                case FUNCT5_AMO_MIN  : ret = AMOMIN_W__; break;
                case FUNCT5_AMO_MAX  : ret = AMOMAX_W__; break;
                case FUNCT5_AMO_MINU : ret = AMOMINU_W_; break;
                case FUNCT5_AMO_MAXU : ret = AMOMAXU_W_; break;
            }
            break;
        }
        case OPCODE_SYSTEM : {
            switch (funct3) {
                case FUNCT3_CSRRW  : ret = CSRRW_____; break;
                case FUNCT3_CSRRS  : ret = CSRRS_____; break;
                case FUNCT3_CSRRC  : ret = CSRRC_____; break;
                case FUNCT3_CSRRWI : ret = CSRRWI____; break;
                case FUNCT3_CSRRSI : ret = CSRRSI____; break;
                case FUNCT3_CSRRCI : ret = CSRRCI____; break;
                case FUNCT3_PRIV   : {
                    switch (funct12) {
                        case FUNCT12_ECALL  : ret = ECALL_____; break;
                        case FUNCT12_EBREAK : ret = EBREAK____; break;
                        case FUNCT12_URET   : ret = URET______; break;
                        case FUNCT12_SRET   : ret = SRET______; break;
                        case FUNCT12_MRET   : ret = MRET______; break;
                        case FUNCT12_WFI    : ret = WFI_______; break;
                        default: {
