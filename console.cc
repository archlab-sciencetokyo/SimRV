...
int Console::recieve_input(){
    ...
    if (select(stdin_fd + 1, &rfds, &wfds, &efds, &tv)>0 && FD_ISSET(stdin_fd, &rfds)){
        ...
        if (r_len!=1) { printf("__ ERROR: in console input\n"); exit(0); }
...
uint32_t Console::console_read(uint32_t offset){
    uint32_t rdata = 0;
    switch(offset) {
    case 0x000: rdata = CONSOLE_MAGIC_VALUE;       break;
    case 0x004: rdata = CONSOLE_VERSION;           break;
    case 0x008: rdata = CONSOLE_DEVIDE_ID;         break;
    case 0x00c: rdata = CONSOLE_VENDOR_ID;         break;
    case 0x010: rdata = CONSOLE_DEVICE_FEATURES;   break;
    case 0x034: rdata = CONSOLE_QUEUE_NUM_MAX;     break;
    case 0x0fc: rdata = CONSOLE_CONFIG_GENERATION; break;
    case 0x044: rdata = Queue[QueueSel].Ready;     break;
    case 0x060: rdata = InterruptStatus;           break;
    case 0x070: rdata = Status;                    break;
    default: break;//{ printf("__ Error: console_read() default %x.\n", offset); exit(0); }
    }
...
void Console::console_write(CPU *cpu, uint32_t offset, uint32_t wdata){
    switch (offset) {
    case 0x030: QueueSel                 = wdata; break;
    ...
    case 0x050: { 
        ...
        if (wdata>1) { printf("__ ERROR: wrong value console_write()\n"); exit(0);}
        if (wdata==1) {
            if(mm.s_use_uc) {
                cc.pc = 0;
                cc.Qnum = wdata;
    ...
    case 0x064: {
        ...
        if (InterruptStatus==0) { cpu->plic_set_irq(VIRTIO_CONSOLE_IRQ, 0); }
        break; }
    }
}
