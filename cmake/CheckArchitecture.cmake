...
foreach(forbidden IN ITEMS "LINK_GROUP" "SIMRV_SOURCES"
                           "SIMRV_LEGACY_TARGET_GRAPH")
  if(top_level_cmake MATCHES "${forbidden}")
    message(
      FATAL_ERROR "Forbidden legacy build construct remains: ${forbidden}")
  endif()
endforeach()
...
foreach(source IN LISTS adapter_files)
  ...
  if(contents
     MATCHES
     "machine_?\\.(cpu|mmem|secondary_harts_|uart|tui|tracer|symbols|breakpoints|gdb_stub|spike_lockstep|s_platform_profile|s_net_mode)([^A-Za-z0-9_]|$)"
  )
    message(FATAL_ERROR "Adapter bypasses Machine capabilities: ${source}")
  endif()
endforeach()

# MachineConfig is the only architectural configuration surface.  Keep the one
# latched MMU telemetry bit private/runtime-only, but reject reintroduction of
# legacy `s_*` option mirrors.
...
if(machine_header
  ...
  message(
    FATAL_ERROR
      "Legacy Machine configuration mirror reintroduced; use MachineConfig")
endif()

if(NOT top_level_cmake MATCHES "install\\([ \t\r\n]*DIRECTORY include/simrv")
