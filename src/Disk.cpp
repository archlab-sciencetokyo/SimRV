...
void update_descriptor(uint32_t desc_idx, uint32_t desc_len, int q_num, QueueState* qs,
                       uint8_t* mmem) {
...
void disk_request(uint8_t* mmem, uint8_t* mdsk, int q_num, QueueState* qs) {
    ...
    while (qs->last_avail_idx != avail_idx) { /* header -> sector -> footer */
        ...
        p = (uint8_t*)&desc;
        for (int i = 0; i < DESC_SIZE; i++) {
            *p = ram_ld(desc_adr_sector + i, 1, mmem);
            p++;
        }
        ...
        p = (uint8_t*)&desc;
        for (int i = 0; i < DESC_SIZE; i++) {
            *p = ram_ld(desc_adr_footer + i, 1, mmem);
            p++;
        }
