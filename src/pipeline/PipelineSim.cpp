...
PipelineSim::PipelineSim() { init_model(); }
...
auto PipelineSim::cycle_count() const -> uint64_t {
    return model_ ? model_->get_stats().cycle_count : 0;
}
auto PipelineSim::stall_cycles() const -> uint64_t {
    return model_ ? model_->get_stats().stall_cycles : 0;
}
auto PipelineSim::bubble_cycles() const -> uint64_t {
    return model_ ? model_->get_stats().bubble_cycles : 0;
}
auto PipelineSim::icache_stalls() const -> uint64_t {
    return model_ ? model_->get_stats().icache_stalls : 0;
}
auto PipelineSim::dcache_stalls() const -> uint64_t {
    return model_ ? model_->get_stats().dcache_stalls : 0;
}
auto PipelineSim::tlb_stalls() const -> uint64_t {
    return model_ ? model_->get_stats().tlb_stalls : 0;
}
auto PipelineSim::structural_stalls() const -> uint64_t {
    return model_ ? model_->get_stats().structural_stalls : 0;
}
auto PipelineSim::data_hazard_stalls() const -> uint64_t {
    return model_ ? model_->get_stats().data_hazard_stalls : 0;
}
auto PipelineSim::control_hazard_bubbles() const -> uint64_t {
    return model_ ? model_->get_stats().control_hazard_bubbles : 0;
}
...
auto PipelineSim::icache_stall_remaining() const -> uint32_t {
    return model_ ? model_->icache_stall_remaining() : 0;
}
auto PipelineSim::dcache_stall_remaining() const -> uint32_t {
    return model_ ? model_->dcache_stall_remaining() : 0;
}
auto PipelineSim::tlb_stall_remaining() const -> uint32_t {
    return model_ ? model_->tlb_stall_remaining() : 0;
}
