...
bool Mmu::translate(Address v_addr, Address* p_addr, PteAccess access) {
...
Word Mmu::read_level1_pte_with_mirror(Address root_page_table_addr, Word vpn_level1,
                                      Word* out_pte_addr) {
...
bool Mmu::validate_pte_permissions(Word pte, Word permission_bits, PteAccess access) const {
    ...
    if (cpu_.priv == kPrivSupervisor && (pte & enum_mask(PteFlag::U)) &&
        !(cpu_.mstatus & enum_mask(MstatusBit::Sum))) {
        return false;
    }

    // User accessing supervisor page
    if (cpu_.priv == kPrivUser && !(pte & enum_mask(PteFlag::U))) {
        return false;
    }
...
void Mmu::update_pte_access_bits(Address pte_addr, Word& pte_value, PteAccess access) {
    ...
    if (updated_pte != pte_value) {
        ...
        constexpr Word kPteSize = 4;
        for (int byte_offset = 0; byte_offset < kPteSize; ++byte_offset) {
...
bool Mmu::page_walk(Address v_addr, Address* p_addr, PteAccess access) {
    ...
    const Word l1_permission_bits =
        (cpu_.mstatus & enum_mask(MstatusBit::Mxr) ? l1_pte >> 1 | l1_pte >> 3 : l1_pte >> 1) &
        kPermissionBitsMask;
    ...
    const Word l0_permission_bits =
        (cpu_.mstatus & enum_mask(MstatusBit::Mxr) ? l0_pte >> 1 | l0_pte >> 3 : l0_pte >> 1) &
        kPermissionBitsMask;
