...
auto parse_register_name(std::string_view input) -> std::optional<ParsedReg> {
    ...
    if ((str.starts_with('x') || str.starts_with('r')) && str.size() > 1 &&
        ...
        if (idx >= 0 && idx < 32) {
            return ParsedReg{.type = RegType::GPR, .index = static_cast<uint8_t>(idx), .canonical_name = std::format("x{}", idx)};
        }
    }
    ...
    for (size_t i = 0; i < kGprAbi.size(); ++i) {
        if (str == kGprAbi[i]) {
            return ParsedReg{.type = RegType::GPR, .index = static_cast<uint8_t>(i), .canonical_name = std::format("x{} ({})", i, kGprAbi[i])};
        }
    }
    if (str == "fp") {
    ...
    if (str.starts_with('f') && str.size() > 1 &&
        ...
        int idx = std::stoi(str.substr(1));
        if (idx >= 0 && idx < 32) {
            return ParsedReg{.type = RegType::FPR, .index = static_cast<uint8_t>(idx), .canonical_name = std::format("f{}", idx)};
        }
    }
    ...
    for (size_t i = 0; i < kFprAbi.size(); ++i) {
        if (str == kFprAbi[i]) {
            return ParsedReg{.type = RegType::FPR, .index = static_cast<uint8_t>(i), .canonical_name = std::format("f{} ({})", i, kFprAbi[i])};
        }
    }
    ...
    if (str.starts_with('v') && str.size() > 1 &&
        ...
        int idx = std::stoi(str.substr(1));
        if (idx >= 0 && idx < 32) {
            return ParsedReg{.type = RegType::VEC, .index = static_cast<uint8_t>(idx), .canonical_name = std::format("v{}", idx)};
        }
    }
...
void BreakpointManager::add_reg_watchpoint(RegType reg_type, uint8_t reg_index, const std::string& reg_name) {
...
void BreakpointManager::remove_reg_watchpoint(RegType reg_type, uint8_t reg_index) {
    std::erase_if(watchpoints_, [reg_type, reg_index](const Watchpoint& wp) -> bool {
        return wp.target == WatchTarget::Register && wp.reg_type == reg_type && wp.reg_index == reg_index;
    });
    update_active();
}
...
auto BreakpointManager::check_reg_changes(const simrv::core::ArchState& state, const simrv::core::ArchState& prev_state) const -> std::optional<BreakpointHit> {
    for (const auto& wp : watchpoints_) {
        ...
        if (wp.reg_type == RegType::GPR) {
            ...
            if (old_val != new_val) {
                return BreakpointHit{
                    ...
                    .description = std::format("Register Watchpoint hit: {} changed from 0x{:x} to 0x{:x}",
            ...
            if (old_val != new_val) {
                return BreakpointHit{
                    .reason = BreakpointHit::Reason::Watchpoint,
                    .addr = static_cast<Address>(wp.reg_index),
                    .description = std::format("Register Watchpoint hit: {} changed from 0x{:016x} to 0x{:016x}",
            ...
            if (vec_changed) {
                return BreakpointHit{
                    .reason = BreakpointHit::Reason::Watchpoint,
                    .addr = static_cast<Address>(wp.reg_index),
                    .description = std::format("Register Watchpoint hit: {} (Vector) value modified",
            ...
            if (prev_state.pc != state.pc) {
                return BreakpointHit{
                    .reason = BreakpointHit::Reason::Watchpoint,
                    .addr = state.pc,
                    .description = std::format("Register Watchpoint hit: PC changed from 0x{:08x} to 0x{:08x}",
