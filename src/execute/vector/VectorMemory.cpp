...
void execute_vl_whole(core::CPU& cpu, simrv::memory::MemorySubsystem& mem, RegId rd, Register base_addr, uint32_t nr) {
    ...
    for (uint32_t i = 0; i < total_bytes; i++) {
        ...
        cpu.state().regs.read_vector(static_cast<RegId>(reg_idx)).u8[i % cpu.state().regs.vlen_bytes()] = static_cast<uint8_t>(val);
    }
}

// Whole vector store helper
void execute_vs_whole(core::CPU& cpu, simrv::memory::MemorySubsystem& mem, RegId vs3, Register base_addr, uint32_t nr) {
    uint32_t total_bytes = nr * cpu.state().regs.vlen_bytes();
    for (uint32_t i = 0; i < total_bytes; i++) {
        Address addr = base_addr + i;
        uint32_t reg_idx = (static_cast<uint32_t>(vs3) + (i / cpu.state().regs.vlen_bytes())) % 32;
        uint8_t val = cpu.state().regs.read_vector(static_cast<RegId>(reg_idx)).u8[i % cpu.state().regs.vlen_bytes()];
        simrv::memory::MemoryAccess::storeInt(mem, cpu, addr, static_cast<Word>(val), isa::Funct3::Sb);
        if (cpu.pipeline_context.pending_exception.has_value()) {
            return;
        }
    }
...
void ExecuteUnit::execute_vector_memory(core::CPU& cpu, core::Machine& machine, isa::OperationId op_id,
