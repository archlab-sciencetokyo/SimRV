...
inline T get_group_element(const core::RegisterFile& regs, RegId base_reg, uint32_t i) {
    ...
    if constexpr (std::is_same_v<T, float>) {
        ...
        return static_cast<T>(
            vreg.u8[elem_idx]);  // NOLINT(cppcoreguidelines-pro-type-union-access)
    } else if constexpr (sizeof(T) == 2) {
        return static_cast<T>(
            vreg.u16[elem_idx]);  // NOLINT(cppcoreguidelines-pro-type-union-access)
    } else if constexpr (sizeof(T) == 4) {
        return static_cast<T>(
            vreg.u32[elem_idx]);  // NOLINT(cppcoreguidelines-pro-type-union-access)
    } else {
        return static_cast<T>(
            vreg.u64[elem_idx]);  // NOLINT(cppcoreguidelines-pro-type-union-access)
    }
}
...
inline void set_group_element(core::RegisterFile& regs, RegId base_reg, uint32_t i, T val) {
    ...
    if constexpr (std::is_same_v<T, float>) {
        ...
        vreg.u8[elem_idx] =
            static_cast<uint8_t>(val);  // NOLINT(cppcoreguidelines-pro-type-union-access)
    } else if constexpr (sizeof(T) == 2) {
        vreg.u16[elem_idx] =
            static_cast<uint16_t>(val);  // NOLINT(cppcoreguidelines-pro-type-union-access)
    } else if constexpr (sizeof(T) == 4) {
        vreg.u32[elem_idx] =
            static_cast<uint32_t>(val);  // NOLINT(cppcoreguidelines-pro-type-union-access)
    } else {
        vreg.u64[elem_idx] =
            static_cast<uint64_t>(val);  // NOLINT(cppcoreguidelines-pro-type-union-access)
    }
}
...
template <typename T, typename Op>
inline void perform_vi(core::CPU& cpu, RegId rd, int32_t imm, RegId rs2, bool vm, uint32_t vl,
                       Op op) {
    const auto& mask_reg = cpu.state().regs.read_vector(RegId::Zero);
...
// Vector comparisons
template <typename T, typename Op>
inline void perform_compare_vv(core::CPU& cpu, RegId rd, RegId rs1, RegId rs2, bool vm, uint32_t vl,
                               Op op) {
...
template <typename T, typename Op>
inline void perform_compare_vx(core::CPU& cpu, RegId rd, Register rs1_val, RegId rs2, bool vm,
                               uint32_t vl, Op op) {
    auto& dest = cpu.state().regs.read_vector(rd);
    const auto& mask_reg = cpu.state().regs.read_vector(RegId::Zero);
    T val1 = static_cast<T>(rs1_val);
...
template <typename T, typename Op>
inline void perform_compare_vi(core::CPU& cpu, RegId rd, int32_t imm, RegId rs2, bool vm,
                               uint32_t vl, Op op) {
    auto& dest = cpu.state().regs.read_vector(rd);
    const auto& mask_reg = cpu.state().regs.read_vector(RegId::Zero);
    T val1 = static_cast<T>(imm);
...
inline T execute_vsmul_element(T vs2, T vs1, uint32_t vxrm, bool& saturated) {
    static_assert(std::is_signed_v<T>);
    using DoubleWidth = std::conditional_t<
        sizeof(T) == 1, int16_t,
