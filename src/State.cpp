...
uint32_t CPU::read_csr(uint32_t addr) {
    ...
    switch (addr) {
        ...
        case CSR_TIMEH:
            rcsr = (uint32_t)(mtime >> 32);
            break;  // 0xC81

        case CSR_SSTATUS:
            rcsr = get_mstatus(0x000de133);
            break;  // 0x100
        case CSR_MSTATUS:
            rcsr = get_mstatus(0xffffffff);
            break;  // 0x300
...
void CPU::write_csr(uint32_t addr, uint32_t wdata) {
    ...
    switch (addr) {
        ...
        case CSR_SIE:
            mie = (mie & ~mideleg) | (wdata & mideleg);
            break;
        case CSR_SIP:
            mip = (mip & ~mideleg) | (wdata & mideleg);
            break;
        case CSR_MEDELEG:
            medeleg = (medeleg & ~mask1) | (wdata & mask1);
            break;
        case CSR_MIDELEG:
            mideleg = (mideleg & ~mask2) | (wdata & mask2);
            break;
        case CSR_MIE:
            mie = (mie & ~mask3) | (wdata & mask3);
            break;
        case CSR_MIP:
            mip = (mip & ~mask4) | (wdata & mask4);
            break;
        ...
        case CSR_MSTATUS:
            ...
            break;  // 0x300
        case CSR_SSTATUS:
            set_mstatus((mstatus & ~SSTATUS_MASK) | (wdata & SSTATUS_MASK));
            break;  // 0x100
        default:
            break;
    }
...
void CPU::raise_exception(uint32_t cause, uint32_t tval) {
    ...
    if (priv <= PRIV_S) {
        if (cause & CAUSE_INTERRUPT) {
            deleg = (mideleg >> (cause & 0x1F)) & 1;
        } else {
            deleg = (medeleg >> (cause & 0x1F)) & 1;
        }
    } else {
        deleg = 0;
    }
