...
/***** load the binary image file and initialize memory                               *****/
/******************************************************************************************/
void load_initram(char *fname, uint8_t *ram){
    ...
    int i=0;
    while(!feof(fp)){ ram[i++] = getc(fp); }
    fclose(fp);
}

/***** load the memory initialize file                                                *****/
/******************************************************************************************/
void load_initmem(std::string file, uint8_t *ram){
    ...
    int file_size = 0;
    while (!ifs.eof()) { ifs.read((char *)(&ram[file_size++]), sizeof(uint8_t)); }
}
...
uint32_t ram_read(uint32_t addr, uint32_t funct3, uint8_t *ram){
    uint32_t rdata = 0;
    int n = (1 << (funct3 & 0x3));
    for (int i=0; i<n; i++) { rdata |= ((uint32_t)ram[(addr + i) & DRAM_MASK]) << (8*i); }
...
uint32_t disk_read(uint32_t addr, uint32_t n, uint8_t *dsk){
    if(n!=1 && n!=2 && n!=4){printf("__ Error: dsk_r() not supported n=%d\n", n); exit(0);}
    uint32_t data = 0;

    for (int i=0; i<(int)n; i++) { data |= ((uint32_t)dsk[(addr + i)]) << (8*i); }
    return data;
}
...
uint32_t queue_read(uint32_t addr, QueueState *q){
    ...
    int idx = addr / 0x24;
    switch (addr % 0x24) {
    case 0x0:  rdata = q[idx].Ready;     break;
    case 0x4:  rdata = q[idx].Notify;    break;
    case 0x8:  rdata = q[idx].DescLow;   break;
    case 0xc:  rdata = q[idx].DescHigh;  break;
    case 0x10: rdata = q[idx].AvailLow;  break;
    case 0x14: rdata = q[idx].AvailHigh; break;
    case 0x18: rdata = q[idx].UsedLow;   break;
    case 0x1c: rdata = q[idx].UsedHigh;  break;
    case 0x20: rdata = q[idx].last_avail_idx; break;
...
void queue_write(uint32_t addr, uint32_t wdata, QueueState *q){
    int idx = addr / 0x24;
    switch (addr % 0x24)  {
    case 0x0:  q[idx].Ready     = wdata; break;
    case 0x4:  q[idx].Notify    = wdata; break;
    case 0x8:  q[idx].DescLow   = wdata; break;
    case 0xc:  q[idx].DescHigh  = wdata; break;
    case 0x10: q[idx].AvailLow  = wdata; break;
    case 0x14: q[idx].AvailHigh = wdata; break;
    case 0x18: q[idx].UsedLow   = wdata; break;
    case 0x1c: q[idx].UsedHigh  = wdata; break;
    case 0x20: q[idx].last_avail_idx = wdata; break;
    default:
        break;
    }
    return;
...
int page_walk(uint32_t v_addr, uint32_t *p_addr, PTE_ACCESS access, CPU *cpu, uint8_t *mmem){
    ...
    uint32_t L1_write    = !(L1_pte & PTE_A_MASK) || (!(L1_pte & PTE_D_MASK) && access==ACCESS_WRITE);
    uint32_t L1_success = !(L1_xwr == 2 || L1_xwr == 6 ||
                            (cpu->priv == PRIV_S && ((L1_pte & PTE_U_MASK) && !(cpu->mstatus & MSTATUS_SUM))) ||
                            (cpu->priv == PRIV_U && (!(L1_pte & PTE_U_MASK))) ||
                            ((L1_xwr >> access) & 1) == 0 );
    /* level 0 */
    ...
    uint32_t L0_write    = !(L0_pte & PTE_A_MASK) || (!(L0_pte & PTE_D_MASK) && access==ACCESS_WRITE);
    uint32_t L0_success = !(L0_xwr == 2 || L0_xwr == 6 ||
                            ((cpu->priv == PRIV_S) && ((L0_pte & PTE_U_MASK) && !(cpu->mstatus & MSTATUS_SUM))) ||
                            ((cpu->priv == PRIV_U) && (!(L0_pte & PTE_U_MASK))) ||
                            ((L0_xwr >> access) & 1) == 0 );
    /* success */
    int ret = 0;
    if (!(L1_pte & PTE_V_MASK))      ret = -1;
    else if (L1_xwr != 0)            ret = L1_success ? 0 : -1;
    else if (!(L0_pte & PTE_V_MASK)) ret = -1;
    else if (L0_xwr != 0)            ret = L0_success ? 0 : -1;
    else                             ret = -1;
    
    /* phys_addr */
    if (ret)             *p_addr = 0;
    else if (L1_success) *p_addr = L1_p_addr;
    else if (L0_success) *p_addr = L0_p_addr;
    ...
    int we = ((L1_xwr != 0 && L1_success) && (L1_write)) || ((L0_xwr != 0 && L0_success) && (L0_write));
...
uint32_t Machine::target_read(uint32_t v_addr, uint32_t funct3){
    ...
    if (cpu->pending_exception == ~0u)
        ...
        case 0x30000000: break;
        case 0x70000000: break;
    ...
    if (cpu->pending_exception == ~0u)
        ...
        case 0x40000000: { ///// VirtIO
            if ((p_addr >> 24) == 0x40) { // console
                switch (p_addr) {
                case 0x40008000: { tohost = wdata; break; }
                default: { console->console_write(cpu, offset, wdata); break; }
                }
            }
...
/******************************************************************************************/
// void initfile_genS(CPU *s, uint8_t *ram, Console *cons, Disk *disk, uint8_t *sector){
//     FILE *f = fopen("xinitmem.bin", "wb"); /***** memory initialize file  *****/
//     fwrite(ram, sizeof(uint8_t), DRAM_SIZE, f);
//     fclose(f);
//     f = fopen("xinitdisk.bin", "wb"); /***** memory initialize file  *****/
//     fwrite(sector, sizeof(uint8_t), DISK_SIZE, f);
//     fclose(f);

//     printf("\n__ file initmem.bin and initdisk.bin were generated after %ld cycle\n", s->mtime);

//     f = fopen("xinitreg.txt", "wb"); /***** register initialize file *****/
//     fprintf(f, "cpu->pc=0x%08x;\n",  s->pc);
//     for(int i=1; i<32; i++) fprintf(f, "cpu->reg[%d]=0x%08x;\n", i, s->reg[i]);
//     fprintf(f, "cpu->mstatus     =0x%08x;\n", s->mstatus);
...
//         fprintf(f, "console->Queue[%d].Notify         =0x%08x;\n", i, cons->Queue[i].Notify);
//         fprintf(f, "console->Queue[%d].DescLow        =0x%08x;\n", i, cons->Queue[i].DescLow);
//         fprintf(f, "console->Queue[%d].DescHigh       =0x%08x;\n", i, cons->Queue[i].DescHigh);
//         fprintf(f, "console->Queue[%d].AvailLow       =0x%08x;\n", i, cons->Queue[i].AvailLow);
//         fprintf(f, "console->Queue[%d].AvailHigh      =0x%08x;\n", i, cons->Queue[i].AvailHigh);
//         fprintf(f, "console->Queue[%d].UsedLow        =0x%08x;\n", i, cons->Queue[i].UsedLow);
//         fprintf(f, "console->Queue[%d].UsedHigh       =0x%08x;\n", i, cons->Queue[i].UsedHigh);
//         fprintf(f, "console->Queue[%d].last_avail_idx =0x%08x;\n", i, cons->Queue[i].last_avail_idx);
//     }
...
//         fprintf(f, "disk->Queue[%d].Notify         =0x%08x;\n", i, disk->Queue[i].Notify);
//         fprintf(f, "disk->Queue[%d].DescLow        =0x%08x;\n", i, disk->Queue[i].DescLow);
//         fprintf(f, "disk->Queue[%d].DescHigh       =0x%08x;\n", i, disk->Queue[i].DescHigh);
//         fprintf(f, "disk->Queue[%d].AvailLow       =0x%08x;\n", i, disk->Queue[i].AvailLow);
//         fprintf(f, "disk->Queue[%d].AvailHigh      =0x%08x;\n", i, disk->Queue[i].AvailHigh);
//         fprintf(f, "disk->Queue[%d].UsedLow        =0x%08x;\n", i, disk->Queue[i].UsedLow);
//         fprintf(f, "disk->Queue[%d].UsedHigh       =0x%08x;\n", i, disk->Queue[i].UsedHigh);
//         fprintf(f, "disk->Queue[%d].last_avail_idx =0x%08x;\n", i, disk->Queue[i].last_avail_idx);
//     }
...
/******************************************************************************************/
void Machine::exec(){
    if(s_gen_binfile) { binfile_gen(cpu, mmem, disk->sector); }
    while(r_running){
        INI();  /* Initialize                    */
...
//    if(cpu->mtime==s_memimg)  { initfile_gen(cpu, mmem); }
    if(cpu->mtime==s_memimg)  { initfile_gen2(cpu, mmem, console, disk, disk->sector); }
...
//        fprintf(f, "TC PC jump_or_branch b_taken jump branch\n");
    ...
    int ir_jb = (r_opcode==OPCODE_JAL)||(r_opcode==OPCODE_JALR)||(r_opcode==OPCODE_BRANCH);
    ...
    fprintf(f, "%08d %08x %08x %d %d %d %d\n", 
            (int)mm.cpu->mtime, cpc, targ, ir_jb, r_tkn, ir_jump, ir_branch);
    fflush(f);
}
...
void Machine::FIN(){ ///// the last stage
    if (s_strace!=0 && cpu->mtime>=s_strace) gen_traces(r_cpc);
    if (cpu->mtime >= s_trace_begin && cpu->mtime <= s_trace_end) trace_output();
    if (cpu->mtime >= s_fincnt-1) { printf("\n__finished by -e option\n"); r_running=0; }
    if (s_bp_trace) gen_bp_traces(r_cpc, r_jmp_pc, r_opcode, r_tkn);

    /***** these are for the application mode *****/
    if ((tohost>>16)==CMD_POWER_OFF) { printf("\n__ Power off\n"); r_running=0; }
    if ((tohost>>16)==CMD_PRINT_CHAR){ printf("%c", (char)(tohost & 0xff)); tohost=0; fflush(stdout); }
}
...
void Machine::IFB(int state){ /***** page walk and TLB update *****/
    ...
    uint32_t  w_padr = (state==1) ?  r_padr1 :  r_padr2;
    uint32_t *r_padr = (state==1) ? &r_padr1 : &r_padr2;
...
void Machine::IFC(){
    ...
    uint32_t ir_l = ram_read(r_padr1, FUNCT3_LHU, mmem); /* Note !! */
    uint32_t ir_h = ram_read(r_padr2, FUNCT3_LHU, mmem); /* Note !! */
    r_ir_org = (ir_h << 16) | (ir_l & 0xFFFF);
}
...
/******************************************************************************************/
void Machine::CVT(){
    uint32_t w_ir_tmp = CB_inst_decomp(r_ir_org);
    uint32_t w_op  = w_ir_tmp & 0x7F;
    uint32_t w_nop = (w_op==OPCODE_OP_FP || w_op==OPCODE_LOAD_FP || w_op==OPCODE_STORE_FP);
    r_ir     = (w_nop) ? RV32_NOP : w_ir_tmp;
    r_cinsn = ((r_ir_org & 3) != 3); /* set if a compressed instruction */
    if (s_use_mix) e_instmix[decoder(r_ir)]++;
}
...
void Machine::OF_(){
    uint32_t w_csr_addr =
        (r_funct3  != FUNCT3_PRIV  ) ? r_funct12 :
        (r_funct12 == FUNCT12_ECALL) ? CSR_MTVEC :
        (r_funct12 == FUNCT12_URET ) ? CSR_UEPC  :
        (r_funct12 == FUNCT12_SRET ) ? CSR_SEPC  :
        (r_funct12 == FUNCT12_MRET ) ? CSR_MEPC  : 0;
    r_rrs1 = cpu->reg[r_rs1]; /* regfile read port 1 */
    r_rrs2 = cpu->reg[r_rs2]; /* regfile read port 2 */
    r_rcsr = cpu->read_csr(w_csr_addr);        /* note !! */
}
...
void Machine::EX1(){
    switch (r_opcode) {
    case OPCODE_LUI   : { r_tkn=0; r_wb_data = r_imm << 12;  break; }
    case OPCODE_AUIPC : { r_tkn=0; r_wb_data = cpu->pc + (r_imm << 12); break;}
    case OPCODE_JAL   : { r_tkn=1; r_wb_data = cpu->pc + (r_cinsn ? 2 : 4); r_jmp_pc = cpu->pc + r_imm; break; }
    case OPCODE_JALR  : { r_tkn=1; r_wb_data = cpu->pc + (r_cinsn ? 2 : 4); r_jmp_pc = r_rrs1 + r_imm;  break; }
    case OPCODE_OP    : { r_tkn=0; r_wb_data = ALU_IM(r_rrs1, r_rrs2, r_funct3, r_funct7); break; }
    case OPCODE_LOAD  : { r_tkn=0; r_mem_addr = r_rrs1 + r_imm; break; }
    case OPCODE_STORE : { r_tkn=0; r_mem_addr = r_rrs1 + r_imm; break; }
    case OPCODE_MISC_M: { r_tkn=0; break; }
    case OPCODE_BRANCH: { r_tkn=ALU_B(r_rrs1, r_rrs2, r_funct3); r_jmp_pc = cpu->pc + r_imm; break; }
    case OPCODE_OP_IMM: { r_tkn=0; r_funct7 &= (r_funct3 == FUNCT3_ADD) ? 0 : 0x20;
            r_wb_data = ALU_IM(r_rrs1, r_imm, r_funct3, r_funct7);
            break;
    }
    case OPCODE_AMO   : {
        r_tkn = 0; r_mem_addr = r_rrs1;
        if (r_funct5 == FUNCT5_AMO_SC) {
            r_wb_data = !((r_rrs1 == cpu->load_res) && cpu->reserved);
        }
        break;
    }
    case OPCODE_SYSTEM : {
        if (r_funct3 == FUNCT3_PRIV) {
            switch (r_funct12) {
            case FUNCT12_ECALL : {
                r_wb_data_csr = CAUSE_USER_ECALL + cpu->priv;
                cpu->pending_exception = CAUSE_USER_ECALL + cpu->priv; // Note!!
                e_icount++;
                break;
            }
            case FUNCT12_EBREAK : { r_tkn=0; break; }
            case FUNCT12_URET   : { r_tkn=1; r_jmp_pc = r_rcsr; break; }
            case FUNCT12_SRET   : { r_tkn=1; r_jmp_pc = r_rcsr; break; }
            case FUNCT12_MRET   : { r_tkn=1; r_jmp_pc = r_rcsr; break; }
            case FUNCT12_WFI    : { r_tkn=0;                    break; }
            default: {if (r_funct7 == FUNCT7_SFENCE_VMA) { r_tkn = 0; } break; }
            }
        } else {
            r_tkn = 0;
            r_wb_data_csr = ALU_C(r_rcsr, r_rrs1, r_imm, r_funct3);
        }
        break;
    }
    }
...
void Machine::LD_(){
    ...
    if (r_opcode == OPCODE_LOAD || (r_opcode == OPCODE_AMO && r_funct5 != FUNCT5_AMO_SC)) {
        r_mem_rdata = target_read(r_mem_addr, r_funct3); //, cpu, mem, console, disk);
    }

    if (r_opcode == OPCODE_AMO && r_funct5 == FUNCT5_AMO_LR) {
        cpu->load_res = r_mem_addr;
        cpu->reserved = 1;
    }
}
...
/******************************************************************************************/
void Machine::EX2(){
    r_mem_wdata = (r_opcode!=OPCODE_AMO) ? r_rrs2 : ALU_A(r_rrs2, r_mem_rdata, r_funct5);
}
...
void Machine::SD_(){
    ...
    if ((r_opcode == OPCODE_STORE) ||
        (r_opcode == OPCODE_AMO && (r_funct5 == FUNCT5_AMO_SC && !r_wb_data && cpu->reserved)) ||
        (r_opcode == OPCODE_AMO &&  r_funct5 != FUNCT5_AMO_LR && r_funct5 != FUNCT5_AMO_SC)) {
        target_write(r_mem_addr, r_mem_wdata, r_funct3);
    }
    if(r_opcode == OPCODE_AMO && (r_funct5 == FUNCT5_AMO_SC && !r_wb_data && 
...
void Machine::WB_(){
    ...
    if ((r_opcode == OPCODE_LOAD) ||
        (r_opcode == OPCODE_AMO && r_funct5 != FUNCT5_AMO_SC)) {
        wire_wb_r_data = r_mem_rdata;
        wire_wb_r_enable = 1;
    } else if (r_opcode == OPCODE_SYSTEM && r_funct3 != FUNCT3_PRIV) {
        wire_wb_r_data = r_rcsr;
        wire_wb_r_enable = 1;
    } else {
        if ((r_opcode == OPCODE_AMO && r_funct5 == FUNCT5_AMO_SC) ||
            (r_opcode == OPCODE_LUI   ) ||
            (r_opcode == OPCODE_AUIPC ) ||
            (r_opcode == OPCODE_JAL   ) ||
            (r_opcode == OPCODE_JALR  ) ||
            (r_opcode == OPCODE_OP    ) ||
    ...
    if (wire_wb_r_enable && r_rd!=0) {
        cpu->reg[r_rd] = wire_wb_r_data;  /* regifile write port 1 */
    }
}
...
/******************************************************************************************/
void Machine::COM(){
    if(r_cinsn) e_ccount++; /** for evaluation **/

    /***** Control and Status Register (CSRs) **********************/
    if (r_opcode == OPCODE_SYSTEM) {
        if (r_funct3 == FUNCT3_PRIV) {
            switch (r_funct12) {
            case FUNCT12_URET : {              break; }
            case FUNCT12_SRET : { cpu->sret(); break; }
            case FUNCT12_MRET : { cpu->mret(); break; }
            }
        } else { cpu->write_csr(r_funct12, r_wb_data_csr); }
    }
...
//    fprintf(s_fp_trace, "%08ld ", e_icount);
    fprintf(s_fp_trace, "%08x " , r_cpc);
    fprintf(s_fp_trace, "%08x"  , r_ir);
    if (s_rtosmode) {
...
void load_devicetree(uint8_t *ram){
    /***** this data is obtained using simrv.dtb  *****/
    uint32_t tbuf[344] = { /***** 344 x 4 = 1376byte  *****/
        0xedfe0dd0, 0x5a050000, 0x38000000, 0x84040000, 0x28000000, 0x11000000, 0x10000000, 0x0,
        0xd6000000, 0x4c040000, 0x0, 0x0, 0x0, 0x0, 0x1000000, 0x0,
        0x3000000, 0x4000000, 0x0, 0x2000000, 0x3000000, 0x4000000, 0xf000000, 0x2000000,
        0x3000000, 0xf000000, 0x1b000000, 0x6573696b, 0x62616c2d, 0x6d69732c, 0x7672, 0x3000000,
        0xf000000, 0x21000000, 0x6573696b, 0x62616c5f, 0x6d69732c, 0x7672, 0x1000000, 0x73757063,
        0x0, 0x3000000, 0x4000000, 0x0, 0x1000000, 0x3000000, 0x4000000, 0xf000000,
        0x0, 0x3000000, 0x4000000, 0x2c000000, 0xe1f505, 0x1000000, 0x40757063, 0x30,
        0x3000000, 0x4000000, 0x3f000000, 0x757063, 0x3000000, 0x4000000, 0x4b000000, 0x0,
        0x3000000, 0x5000000, 0x4f000000, 0x79616b6f, 0x0, 0x3000000, 0x6000000, 0x21000000,
        0x63736972, 0x76, 0x3000000, 0x9000000, 0x56000000, 0x32337672, 0x6d696361, 0x0,
        0x3000000, 0xb000000, 0x60000000, 0x63736972, 0x76732c76, 0x3233, 0x3000000, 0x4000000,
        0x69000000, 0xe1f505, 0x1000000, 0x65746e69, 0x70757272, 0x6f632d74, 0x6f72746e, 0x72656c6c,
        0x0, 0x3000000, 0x4000000, 0x79000000, 0x1000000, 0x3000000, 0x0, 0x8a000000,
        0x3000000, 0xf000000, 0x21000000, 0x63736972, 0x70632c76, 0x6e692d75, 0x6374, 0x3000000,
        0x4000000, 0x9f000000, 0x1000000, 0x2000000, 0x2000000, 0x2000000, 0x1000000, 0x6f6d656d,
        0x38407972, 0x30303030, 0x303030, 0x3000000, 0x7000000, 0x3f000000, 0x6f6d656d, 0x7972,
        0x3000000, 0x10000000, 0x4b000000, 0x0, 0x80, 0x0, 0x4, 0x2000000,
        0x1000000, 0x636f73, 0x3000000, 0x4000000, 0x0, 0x2000000, 0x3000000, 0x4000000,
        0xf000000, 0x2000000, 0x3000000, 0xb000000, 0x21000000, 0x706d6973, 0x622d656c, 0x7375,
        0x3000000, 0x0, 0xa7000000, 0x1000000, 0x6e696c63, 0x30364074, 0x30303030, 0x3030,
        0x3000000, 0xd000000, 0x21000000, 0x63736972, 0x6c632c76, 0x30746e69, 0x0, 0x3000000,
        0x10000000, 0xae000000, 0x1000000, 0x3000000, 0x1000000, 0x7000000, 0x3000000, 0x10000000,
        0x4b000000, 0x0, 0x60, 0x0, 0x8, 0x2000000, 0x1000000, 0x63696c70,
        0x30303540, 0x30303030, 0x30, 0x3000000, 0x4000000, 0x79000000, 0x1000000, 0x3000000,
        0x0, 0x8a000000, 0x3000000, 0xc000000, 0x21000000, 0x63736972, 0x6c702c76, 0x306369,
        0x3000000, 0x4000000, 0xc2000000, 0x1f000000, 0x3000000, 0x10000000, 0x4b000000, 0x0,
        0x50, 0x0, 0x8, 0x3000000, 0x10000000, 0xae000000, 0x1000000, 0x9000000,
        0x1000000, 0xb000000, 0x3000000, 0x4000000, 0x9f000000, 0x2000000, 0x2000000, 0x1000000,
        0x74726976, 0x34406f69, 0x30303030, 0x303030, 0x3000000, 0xc000000, 0x21000000, 0x74726976,
        0x6d2c6f69, 0x6f696d, 0x3000000, 0x10000000, 0x4b000000, 0x0, 0x40, 0x0,
        0x8, 0x3000000, 0x8000000, 0xae000000, 0x2000000, 0x1000000, 0x2000000, 0x1000000,
        0x74726976, 0x34406f69, 0x30303038, 0x303030, 0x3000000, 0xc000000, 0x21000000, 0x74726976,
        0x6d2c6f69, 0x6f696d, 0x3000000, 0x10000000, 0x4b000000, 0x0, 0x48, 0x0,
        0x8, 0x3000000, 0x8000000, 0xae000000, 0x2000000, 0x2000000, 0x2000000, 0x2000000,
        0x1000000, 0x736f6863, 0x6e65, 0x3000000, 0x1e000000, 0xcd000000, 0x736e6f63, 0x3d656c6f,
        0x30637668, 0x6f6f7220, 0x642f3d74, 0x762f7665, 0x72206164, 0x77, 0x2000000, 0x2000000,
        0x9000000, 0x64646123, 0x73736572, 0x6c65632d, 0x2300736c, 0x657a6973, 0x6c65632d, 0x6d00736c,
        0x6c65646f, 0x6d6f6300, 0x69746170, 0x656c62, 0x656d6974, 0x65736162, 0x6572662d, 0x6e657571,
        0x64007963, 0x63697665, 0x79745f65, 0x72006570, 0x73006765, 0x75746174, 0x69720073, 0x2c766373,
        0x617369, 0x2d756d6d, 0x65707974, 0x6f6c6300, 0x662d6b63, 0x75716572, 0x79636e65, 0x6e692300,
        0x72726574, 0x2d747075, 0x6c6c6563, 0x6e690073, 0x72726574, 0x2d747075, 0x746e6f63, 0x6c6c6f72,
        0x70007265, 0x646e6168, 0x7200656c, 0x65676e61, 0x6e690073, 0x72726574, 0x73747075, 0x7478652d,
        0x65646e65, 0x69720064, 0x2c766373, 0x7665646e, 0x6f6f6200, 0x67726174, 0xff0073, 0x0};
    int *p = (int*)ram;
...
int Machine::init(int argc, char *argv[]){
    ...
    if(s_fn_dvtree==NULL) load_devicetree(mmem+D_INITD_ADDR);
    else load_initram(s_fn_dvtree, mmem+D_INITD_ADDR);

    if(s_use_disk) load_initram(s_fn_dskimg, disk->sector); // load a disk image file

    if(s_use_mix) for (int i=0; i<NUMOFID___; i++) e_instmix[i] = 0;
...
int Microcn::exec(){
    ...
    switch (r_opcode) {
    case OPCODE_LUI   : { r_tkn=0; r_wb_data = r_imm << 12;  break; }
    case OPCODE_AUIPC : { r_tkn=0; r_wb_data = pc + (r_imm << 12); break;}
    case OPCODE_JAL   : { r_tkn=1; r_wb_data = pc + 4; r_jmp_pc = pc + r_imm; break; }
    case OPCODE_JALR  : { r_tkn=1; r_wb_data = pc + 4; r_jmp_pc = r_rrs1 + r_imm;  break; }
    case OPCODE_OP    : { r_tkn=0; r_wb_data = ALU_IM(r_rrs1, r_rrs2, r_funct3, r_funct7); break; }
    case OPCODE_LOAD  : { r_tkn=0; r_mem_addr = r_rrs1 + r_imm; break; }
    case OPCODE_STORE : { r_tkn=0; r_mem_addr = r_rrs1 + r_imm; break; }
    case OPCODE_MISC_M: { r_tkn=0; break; }
    case OPCODE_BRANCH: { r_tkn=ALU_B(r_rrs1, r_rrs2, r_funct3); r_jmp_pc = pc + r_imm; break; }
    case OPCODE_OP_IMM: { r_tkn=0; r_funct7 &= (r_funct3 == FUNCT3_ADD) ? 0 : 0x20;
            r_wb_data = ALU_IM(r_rrs1, r_imm, r_funct3, r_funct7);
            break;}
    }
    ...
    if (r_opcode == OPCODE_LOAD) {
        ...
        else r_mem_rdata = ram_read(r_mem_addr & DRAM_MASK, r_funct3, cmem);
    }
    if (r_opcode == OPCODE_STORE) {
        if(r_mem_addr==0x40008000){
            if(r_rrs2>>16==1){ printf("%c", (char)(r_rrs2 & 0xff)); fflush(stdout); }
            if(r_rrs2>>16==2){ ret=0; }
