/**
 * @file machine.cc
 * @brief Implementation of the machine-related functions.
 *
 * This file contains the implementation of various functions related to the
 * machine, such as generating binary image files, loading memory initialization
 * files, reading from RAM and disk, and performing page walks for Sv32 page
 * translation.
 */
...
/**
 * @brief Set the options for the machine.
 *
 * This function sets the options for the machine based on the command line
 * arguments.
 *
 * @param m Pointer to the Machine object.
...
/**
 * @brief Generate a binary image file for FPGA run.
 *
 * This function generates a binary image file named "inits.bin" based on the
 * provided RAM and sector data. It writes the RAM data, device tree data, and
 * disk sector data to the file.
 *
 * @param s Pointer to the CPU object.
 * @param ram Pointer to the RAM data.
 * @param sector Pointer to the disk sector data.
 */
void binfile_gen(CPU *s, uint8_t *ram, uint8_t *sector);

/**
 * @brief Load the binary image file and initialize memory.
 *
 * This function loads the binary image file specified by the given filename and
 * initializes the RAM with its contents.
 *
 * @param fname Filename of the binary image file.
 * @param ram Pointer to the RAM data.
 */
void load_initram(char *fname, uint8_t *ram);

/**
 * @brief Load the memory initialization file.
 *
 * This function loads the memory initialization file specified by the given
 * filename and initializes the RAM with its contents.
 *
 * @param file Filename of the memory initialization file.
 * @param ram Pointer to the RAM data.
 */
 ...
 *
 * This function reads data from the RAM at the specified address and with the
 * specified access size.
 *
 * @param addr Address to read from.
 * @param funct3 Function code specifying the access size.
 * @param ram Pointer to the RAM data.
 ...
 *
 * This function reads data from the disk at the specified address and with the
 * specified access size.
 *
 * @param addr Address to read from.
 * @param n Access size in bytes (1, 2, or 4).
 * @param dsk Pointer to the disk data.
 * @return The read data.
 */
uint32_t disk_read(uint32_t addr, uint32_t n, uint8_t *dsk);

/**
 * @brief Read data from a queue.
 *
 * This function reads data from the specified address in the queue.
 *
 * @param addr Address to read from.
 * @param q Pointer to the QueueState object.
 * @return The read data.
 */
 ...
 *
 * This function writes data to the specified address in the queue.
 *
...
/**
 * @brief Perform Sv32 page walk.
 *
 * This function performs the Sv32 page walk for the given virtual address and
 * returns the corresponding physical address. It also handles page faults and
 * updates the page table entries if necessary.
 *
 * @param v_addr Virtual address to translate.
 * @param p_addr Pointer to store the physical address.
 * @param access Access type (read or write).
 * @param cpu Pointer to the CPU object.
 * @param mmem Pointer to the main memory data.
 * @return 0 if the translation is successful, -1 if there is a page fault.
 */
int page_walk(uint32_t v_addr, uint32_t *p_addr, PTE_ACCESS access, CPU *cpu,
              uint8_t *mmem);
...
int page_walk(uint32_t v_addr, uint32_t *p_addr, PTE_ACCESS access, CPU *cpu,
  ...
  uint32_t L1_xwr =
      (cpu->mstatus & MSTATUS_MXR ? L1_pte >> 1 | L1_pte >> 3 : L1_pte >> 1) &
      7;
  uint32_t L1_p_addr =
      (v_addr & 0x3FFFFF) | (((L1_pte >> 10) << 12) & ~0x3FFFFF);
  uint32_t L1_write = !(L1_pte & PTE_A_MASK) ||
                      (!(L1_pte & PTE_D_MASK) && access == ACCESS_WRITE);
  uint32_t L1_success =
      !(L1_xwr == 2 || L1_xwr == 6 ||
        (cpu->priv == PRIV_S &&
         ((L1_pte & PTE_U_MASK) && !(cpu->mstatus & MSTATUS_SUM))) ||
  ...
  uint32_t L0_xwr =
      (cpu->mstatus & MSTATUS_MXR ? L0_pte >> 1 | L0_pte >> 3 : L0_pte >> 1) &
      7;
  uint32_t L0_p_addr = (v_addr & 0xFFF) | (((L0_pte >> 10) << 12) & ~0xFFF);
  uint32_t L0_write = !(L0_pte & PTE_A_MASK) ||
                      (!(L0_pte & PTE_D_MASK) && access == ACCESS_WRITE);
  uint32_t L0_success =
      !(L0_xwr == 2 || L0_xwr == 6 ||
        ((cpu->priv == PRIV_S) &&
         ((L0_pte & PTE_U_MASK) && !(cpu->mstatus & MSTATUS_SUM))) ||
  ...
  /* update pte */
  uint32_t L1_pte_write =
      L1_pte | PTE_A_MASK | (access == ACCESS_WRITE ? PTE_D_MASK : 0);
  uint32_t L0_pte_write =
      L0_pte | PTE_A_MASK | (access == ACCESS_WRITE ? PTE_D_MASK : 0);
  int we = ((L1_xwr != 0 && L1_success) && (L1_write)) ||
...
void Machine::target_write(uint32_t v_addr, uint32_t wdata, uint32_t funct3) {
  ...
  if (cpu->pending_exception == ~0u) switch (p_addr & 0xF0000000) {
      ...
      case 0x60000000: {  ///// CLINT (Core Local Interruptor)
        ...
        if (offset == 0x4004) {
          cpu->mtimecmp =
              (cpu->mtimecmp & 0xffffffff) | ((uint64_t)wdata << 32);
          cpu->mip &= ~MIP_MTIP;
        }
        break;
      }
...
void initfile_gen2(CPU *s, uint8_t *ram, Console *cons, Disk *disk,
                   uint8_t *sector) {
  FILE *f = fopen("init_mem.txt", "w"); /***** memory initialize file  *****/
  for (int i = 0; i < DRAM_SIZE; i++) fprintf(f, "%x\n", ram[i]);
  fclose(f);
  ...
  fprintf(f, "p.pc=32'h%08x;\n", s->pc);
  for (int i = 1; i < 32; i++)
    fprintf(f, "p.regs.mem[%d]=32'h%08x;\n", i, s->reg[i]);
  ...
  for (int i = 0; i < TLB_SIZE; i++) {
    fprintf(f, "mmu.TLB_inst_r.r_valid[%d] =%d;\n", i,
            !(s->TLB_inst_r[i].p_addr == -1u));
    fprintf(f, "mmu.TLB_inst_r.mem[%d][39:22] =18'h%05x;\n", i,
            s->TLB_inst_r[i].v_addr >> 14);
    fprintf(f, "mmu.TLB_inst_r.mem[%d][21:0] =22'h%06x;\n", i,
            s->TLB_inst_r[i].p_addr >> 10);
  }
  for (int i = 0; i < TLB_SIZE; i++) {
    fprintf(f, "mmu.TLB_data_r.r_valid[%d] =%d;\n", i,
            !(s->TLB_data_r[i].p_addr == -1u));
    fprintf(f, "mmu.TLB_data_r.mem[%d][39:22] =18'h%05x;\n", i,
            s->TLB_data_r[i].v_addr >> 14);
    fprintf(f, "mmu.TLB_data_r.mem[%d][21:0] =22'h%06x;\n", i,
            s->TLB_data_r[i].p_addr >> 10);
  }
  for (int i = 0; i < TLB_SIZE; i++) {
    fprintf(f, "mmu.TLB_data_w.r_valid[%d] =%d;\n", i,
            !(s->TLB_data_w[i].p_addr == -1u));
    fprintf(f, "mmu.TLB_data_w.mem[%d][39:22] =18'h%05x;\n", i,
            s->TLB_data_w[i].v_addr >> 14);
    fprintf(f, "mmu.TLB_data_w.mem[%d][21:0] =22'h%06x;\n", i,
            s->TLB_data_w[i].p_addr >> 10);
  }
...
void Machine::display_result() {
  ...
  gettimeofday(&t, NULL);
  uint64_t etime =
      (t.tv_sec - s_stime.tv_sec) * 1000000ul + t.tv_usec - s_stime.tv_usec;
...
void Machine::INI() {  ///// the first stage
  ...
  if (cpu->mtime > s_enabletimer) { /***** enable timer after linux boot *****/
    ...
    if ((cpu->mtime & (uint64_t)0xfffff) == 0 &&
        console->fifo_en) {                  // 2019-08-30
...
void gen_bp_traces(uint32_t cpc, uint32_t jmp_pc, int r_opcode, int r_tkn) {
  ...
  int ir_jb = (r_opcode == OPCODE_JAL) || (r_opcode == OPCODE_JALR) ||
              (r_opcode == OPCODE_BRANCH);
  int ir_jump = (r_opcode == OPCODE_JAL)    ? 2
                : (r_opcode == OPCODE_JALR) ? 3
                                            : 0;
  int ir_branch = (r_opcode == OPCODE_BRANCH);

  unsigned int targ = (ir_jump | ir_branch) ? jmp_pc : 0;
  fprintf(f, "%08d %08x %08x %d %d %d %d\n", (int)mm.cpu->mtime, cpc, targ,
...
void Machine::IFA() { /***** address translation *****/
  ...
  if (cpu->priv == PRIV_M ||
      (cpu->satp >> 31) == 0) { /** No translation or protection **/
    w_padr1 = w_vadr1;
    w_padr2 = w_vadr2;
  } else {
    TLBEntry *tlb_e1 =
        &cpu->TLB_inst_r[(w_vadr1 >> D_PAGE_SHIFT) & (TLB_SIZE - 1)];
    TLBEntry *tlb_e2 =
        &cpu->TLB_inst_r[(w_vadr2 >> D_PAGE_SHIFT) & (TLB_SIZE - 1)];
...
void Machine::IFB(int state) { /***** page walk and TLB update *****/
  ...
  if (w_padr == ~0u) {
    ...
    if (pf) {
      ...
      TLBEntry *tlb_e1 =
          &cpu->TLB_inst_r[(w_vadr >> D_PAGE_SHIFT) & (TLB_SIZE - 1)];
...
void Machine::CVT() {
  ...
  uint32_t w_nop = (w_op == OPCODE_OP_FP || w_op == OPCODE_LOAD_FP ||
                    w_op == OPCODE_STORE_FP);
...
void Machine::EX1() {
  switch (r_opcode) {
    ...
    case OPCODE_AUIPC: {
      r_tkn = 0;
      r_wb_data = cpu->pc + (r_imm << 12);
      break;
    }
    case OPCODE_JAL: {
      r_tkn = 1;
      r_wb_data = cpu->pc + (r_cinsn ? 2 : 4);
      r_jmp_pc = cpu->pc + r_imm;
      break;
    }
    case OPCODE_JALR: {
      r_tkn = 1;
    ...
    case OPCODE_OP: {
      r_tkn = 0;
      r_wb_data = ALU_IM(r_rrs1, r_rrs2, r_funct3, r_funct7);
      break;
    }
    case OPCODE_LOAD: {
      r_tkn = 0;
    ...
    case OPCODE_BRANCH: {
      r_tkn = ALU_B(r_rrs1, r_rrs2, r_funct3);
      r_jmp_pc = cpu->pc + r_imm;
      break;
    }
    case OPCODE_OP_IMM: {
...
void Machine::LD_() {
  ...
  if (r_opcode == OPCODE_LOAD ||
    ...
    r_mem_rdata =
        target_read(r_mem_addr, r_funct3);  //, cpu, mem, console, disk);
  }
...
void Machine::EX2() {
  r_mem_wdata =
      (r_opcode != OPCODE_AMO) ? r_rrs2 : ALU_A(r_rrs2, r_mem_rdata, r_funct5);
}
...
void Machine::SD_() {
  ...
  if ((r_opcode == OPCODE_STORE) ||
      (r_opcode == OPCODE_AMO &&
       (r_funct5 == FUNCT5_AMO_SC && !r_wb_data && cpu->reserved)) ||
      (r_opcode == OPCODE_AMO && r_funct5 != FUNCT5_AMO_LR &&
       r_funct5 != FUNCT5_AMO_SC)) {
    target_write(r_mem_addr, r_mem_wdata, r_funct3);
  }
  if (r_opcode == OPCODE_AMO &&
      (r_funct5 == FUNCT5_AMO_SC && !r_wb_data && cpu->reserved &&
...
void Machine::WB_() {
  ...
  if ((r_opcode == OPCODE_LOAD) ||
    ...
    if ((r_opcode == OPCODE_AMO && r_funct5 == FUNCT5_AMO_SC) ||
        ...
        (r_opcode == OPCODE_JAL) || (r_opcode == OPCODE_JALR) ||
        (r_opcode == OPCODE_OP) || (r_opcode == OPCODE_OP_IMM)) {
...
void Machine::COM() {
  ...
  if (r_opcode == OPCODE_SYSTEM) {
    if (r_funct3 == FUNCT3_PRIV) {
      ...
      cpu->write_csr(r_funct12, r_wb_data_csr);
    }
  }
...
void load_devicetree(uint8_t *ram) {
  ...
  uint32_t tbuf[344] = {
      /***** 344 x 4 = 1376byte  *****/
      0xedfe0dd0, 0x5a050000, 0x38000000, 0x84040000, 0x28000000, 0x11000000,
      0x10000000, 0x0,        0xd6000000, 0x4c040000, 0x0,        0x0,
      0x0,        0x0,        0x1000000,  0x0,        0x3000000,  0x4000000,
      0x0,        0x2000000,  0x3000000,  0x4000000,  0xf000000,  0x2000000,
      0x3000000,  0xf000000,  0x1b000000, 0x6573696b, 0x62616c2d, 0x6d69732c,
      0x7672,     0x3000000,  0xf000000,  0x21000000, 0x6573696b, 0x62616c5f,
      0x6d69732c, 0x7672,     0x1000000,  0x73757063, 0x0,        0x3000000,
      0x4000000,  0x0,        0x1000000,  0x3000000,  0x4000000,  0xf000000,
      0x0,        0x3000000,  0x4000000,  0x2c000000, 0xe1f505,   0x1000000,
      0x40757063, 0x30,       0x3000000,  0x4000000,  0x3f000000, 0x757063,
      0x3000000,  0x4000000,  0x4b000000, 0x0,        0x3000000,  0x5000000,
      0x4f000000, 0x79616b6f, 0x0,        0x3000000,  0x6000000,  0x21000000,
      0x63736972, 0x76,       0x3000000,  0x9000000,  0x56000000, 0x32337672,
      0x6d696361, 0x0,        0x3000000,  0xb000000,  0x60000000, 0x63736972,
      0x76732c76, 0x3233,     0x3000000,  0x4000000,  0x69000000, 0xe1f505,
      0x1000000,  0x65746e69, 0x70757272, 0x6f632d74, 0x6f72746e, 0x72656c6c,
      0x0,        0x3000000,  0x4000000,  0x79000000, 0x1000000,  0x3000000,
      0x0,        0x8a000000, 0x3000000,  0xf000000,  0x21000000, 0x63736972,
      0x70632c76, 0x6e692d75, 0x6374,     0x3000000,  0x4000000,  0x9f000000,
      0x1000000,  0x2000000,  0x2000000,  0x2000000,  0x1000000,  0x6f6d656d,
      0x38407972, 0x30303030, 0x303030,   0x3000000,  0x7000000,  0x3f000000,
      0x6f6d656d, 0x7972,     0x3000000,  0x10000000, 0x4b000000, 0x0,
      0x80,       0x0,        0x4,        0x2000000,  0x1000000,  0x636f73,
      0x3000000,  0x4000000,  0x0,        0x2000000,  0x3000000,  0x4000000,
      0xf000000,  0x2000000,  0x3000000,  0xb000000,  0x21000000, 0x706d6973,
      0x622d656c, 0x7375,     0x3000000,  0x0,        0xa7000000, 0x1000000,
      0x6e696c63, 0x30364074, 0x30303030, 0x3030,     0x3000000,  0xd000000,
      0x21000000, 0x63736972, 0x6c632c76, 0x30746e69, 0x0,        0x3000000,
      0x10000000, 0xae000000, 0x1000000,  0x3000000,  0x1000000,  0x7000000,
      0x3000000,  0x10000000, 0x4b000000, 0x0,        0x60,       0x0,
      0x8,        0x2000000,  0x1000000,  0x63696c70, 0x30303540, 0x30303030,
      0x30,       0x3000000,  0x4000000,  0x79000000, 0x1000000,  0x3000000,
      0x0,        0x8a000000, 0x3000000,  0xc000000,  0x21000000, 0x63736972,
      0x6c702c76, 0x306369,   0x3000000,  0x4000000,  0xc2000000, 0x1f000000,
      0x3000000,  0x10000000, 0x4b000000, 0x0,        0x50,       0x0,
      0x8,        0x3000000,  0x10000000, 0xae000000, 0x1000000,  0x9000000,
      0x1000000,  0xb000000,  0x3000000,  0x4000000,  0x9f000000, 0x2000000,
      0x2000000,  0x1000000,  0x74726976, 0x34406f69, 0x30303030, 0x303030,
      0x3000000,  0xc000000,  0x21000000, 0x74726976, 0x6d2c6f69, 0x6f696d,
      0x3000000,  0x10000000, 0x4b000000, 0x0,        0x40,       0x0,
      0x8,        0x3000000,  0x8000000,  0xae000000, 0x2000000,  0x1000000,
      0x2000000,  0x1000000,  0x74726976, 0x34406f69, 0x30303038, 0x303030,
      0x3000000,  0xc000000,  0x21000000, 0x74726976, 0x6d2c6f69, 0x6f696d,
      0x3000000,  0x10000000, 0x4b000000, 0x0,        0x48,       0x0,
      0x8,        0x3000000,  0x8000000,  0xae000000, 0x2000000,  0x2000000,
      0x2000000,  0x2000000,  0x1000000,  0x736f6863, 0x6e65,     0x3000000,
      0x1e000000, 0xcd000000, 0x736e6f63, 0x3d656c6f, 0x30637668, 0x6f6f7220,
      0x642f3d74, 0x762f7665, 0x72206164, 0x77,       0x2000000,  0x2000000,
      0x9000000,  0x64646123, 0x73736572, 0x6c65632d, 0x2300736c, 0x657a6973,
      0x6c65632d, 0x6d00736c, 0x6c65646f, 0x6d6f6300, 0x69746170, 0x656c62,
      0x656d6974, 0x65736162, 0x6572662d, 0x6e657571, 0x64007963, 0x63697665,
      0x79745f65, 0x72006570, 0x73006765, 0x75746174, 0x69720073, 0x2c766373,
      0x617369,   0x2d756d6d, 0x65707974, 0x6f6c6300, 0x662d6b63, 0x75716572,
      0x79636e65, 0x6e692300, 0x72726574, 0x2d747075, 0x6c6c6563, 0x6e690073,
      0x72726574, 0x2d747075, 0x746e6f63, 0x6c6c6f72, 0x70007265, 0x646e6168,
      0x7200656c, 0x65676e61, 0x6e690073, 0x72726574, 0x73747075, 0x7478652d,
      0x65646e65, 0x69720064, 0x2c766373, 0x7665646e, 0x6f6f6200, 0x67726174,
      0xff0073,   0x0};
  int *p = (int *)ram;
...
int Machine::init(int argc, char *argv[]) {
  set_options(this, argc,
              argv); /* set options before the object instantiations */
  ...
  if (s_use_disk)
    load_initram(s_fn_dskimg, disk->sector);  // load a disk image file

  if (s_use_mix)
    for (int i = 0; i < NUMOFID___; i++) e_instmix[i] = 0;

  if (s_rtosmode)
    cpu->misa = 0x00000100;  // RV32i, Machine ISA register when RTOS mode
...
int Microcn::exec() {
  ...
  if (r_opcode == OPCODE_STORE) {
    if (r_mem_addr == 0x40008000) {
      ...
      /*for (int i=0; i<(1 << r_funct3); i++) {
          disk[(r_mem_addr+i) & DISK_MASK] = (uint8_t)((r_rrs2 >> (8*i)) &
      0xFF);
      }*/
    } else if ((r_mem_addr >> 12) == 0x4000a) {
  ...
  if (r_opcode == OPCODE_LOAD) {
    wire_wb_r_data = r_mem_rdata;
    wire_wb_r_enable = 1;
  } else if ((r_opcode == OPCODE_LUI) || (r_opcode == OPCODE_AUIPC) ||
             (r_opcode == OPCODE_JAL) || (r_opcode == OPCODE_JALR) ||
             (r_opcode == OPCODE_OP) || (r_opcode == OPCODE_OP_IMM)) {
    wire_wb_r_data = r_wb_data;
    wire_wb_r_enable = 1;
