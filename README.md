...
SimRV is a cycle-oriented RISC-V functional simulator for RV32 systems with Linux/application execution, VirtIO console and disk emulation, and a regression-friendly development workflow.

&copy; [ArchLab. TokyoTech](https://www.arch.cs.titech.ac.jp)

[![C/C++ CI](https://github.com/archlab-sciencetokyo/SimRV/actions/workflows/c-cpp.yml/badge.svg?branch=main)](https://github.com/archlab-sciencetokyo/SimRV/actions/workflows/c-cpp.yml)
...
1. Read help message of SimCore/RISC-V (SimRV) by
...
1. `2.0.0-alpha.x`: RV32GC stabilization and extension cleanup
    - finalize RV32GC decode/execute behavior and MISA gating
    - complete type and infrastructure refactors for extension-friendly design
2. `2.0.0-beta.x`: RV64GC bring-up on shared architecture
    - remove remaining 32-bit assumptions in core execution/memory paths
    - establish RV64GC-compatible decode/execute/csr behavior
3. `2.0.0-rc.x`: Linux boot reliability and release hardening
    - Linux boot path validation for supported configurations
    - regression expansion and release-blocker fixes only
4. `2.0.0`: stable release
    - RV32GC + RV64GC support with extensibility-focused architecture
    - Linux boot capability with documented workflows
