/**
 * @file define.h
 * @brief Header file containing various definitions and constants for the
 * RISC-V simulation.
 *
 * This file defines various constants, flags, and masks used in the RISC-V
 * simulation. It also includes the necessary header files and enumerations for
 * the simulation.
 *
 * @note This file is part of the SimCore/RISC-V project developed by ArchLab.
 * TokyoTech.
 * @since 2018-07-05
 */
#ifndef __define_h__
...
/******************************************************************************************/
#define D_START_PC \
  (0x80000000)  // initial PC, the simulation starts from this address
...
/******************************************************************************************/
/**
 * @brief Structure representing the state of a queue.
 */
struct QueueState {
  uint32_t Ready;     /**< Number of ready elements in the queue. */
  uint32_t Notify;    /**< Number of notifications for the queue. */
  uint32_t DescLow;   /**< Lower 32 bits of the descriptor address. */
  uint32_t DescHigh;  /**< Upper 32 bits of the descriptor address. */
  uint32_t AvailLow;  /**< Lower 32 bits of the available ring address. */
  uint32_t AvailHigh; /**< Upper 32 bits of the available ring address. */
  uint32_t UsedLow;   /**< Lower 32 bits of the used ring address. */
  uint32_t UsedHigh;  /**< Upper 32 bits of the used ring address. */
  uint32_t
      last_avail_idx; /**< Index of the last available element in the queue. */
};

/**
 * @brief Structure representing the header of a block request.
 */
...
/**
 * @brief Represents a descriptor.
 *
 * This struct contains information about a descriptor, including its address,
 * length, flags, and the index of the next descriptor.
 */
struct Descriptor {
  uint64_t adr;   /**< The address of the descriptor. */
  uint32_t len;   /**< The length of the descriptor. */
  uint16_t flags; /**< The flags associated with the descriptor. */
  uint16_t next;  /**< The index of the next descriptor. */
};
